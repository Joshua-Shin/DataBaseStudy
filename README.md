...
- 데이터 모델과 성능
  ...
  - 반정규화를 한다고 해서 꼭 정규형 수치가 낮아지는건 아니고, 넓은 의미에서는 정규화 시킬 수 있는 상태인데 그렇게 하지 않는 상태로 돌리는것 까지 포함함.
  - 예를 들어 테이블 수직 분할. 딱히 정규형이 낮아지는것이 아닌데도 반정규화 종류중 하나임.
  - 성능 이슈는 삽입 갱신 삭제에서 나타나지 않고, 조회할때, 상황에 따라 너무 정규화된 상태면 조인을 여러번 해야된다든지 계산을 계속해야된다든지 하는 이유 때문에 생길 수 있어
  - 이상 현상. 삽입, 삭제, 갱신 이상이 있어.
  ...
  - PK가 composite key일 경우 어떤 요소를 먼저 두나에 따라 정렬 우선순위가 정해짐. 따라서, 조회를 많이 하는 애를 composite key중 위로 두는게 좋아
